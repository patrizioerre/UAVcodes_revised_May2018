function [cL,Lift,Drag,dp,d,roll,yaw,L_panel]=force_calc_patrizio(Nx,Ny,a,a_d,w_ind_drag,b,G,Gs,t,dt,S,dl_x,dly,uinf,aoa,Lam,dih,aoaf_L,aoaf_R,rudder) %codegen
...
for i=1:Nx
    for j=1:2*Ny
        ...
        dzj=-dly*(sin(dih(i,j))-sin(aoa(i,j))*sin(Lam(i,j)));
        dyj=-dly*cos(Lam(i,j))*cos(dih(i,j));
        dxj=-dly*sin(Lam(i,j))*cos(aoa(i,j));
        vj=[dxj,dyj,dzj]/norm([dxj,dyj,dzj]);
        % for i direction (chord wise)
        dxi=dl_x(1)*cos(aoa(i,j));
        dyi=0;
        dzi=-dl_x(1)*sin(aoa(i,j))*cos(dih(i,j));
        ...
        Tau_i_j(i,j)=vi(2);
        Tau_i_k(i,j)=vi(3);
        Tau_j_j(i,j)=vj(2); % since tangent in y direction does not change sign from left to right
...
dxi_L= dl_x(1)*cos(aoa(Nx,1));
dzi_L= -dl_x(1)*sin(aoa(Nx,1))*cos(dih(Nx,1));
vi_L=[dxi_L,dyi,dzi_L]/norm([dxi_L,dyi,dzi_L]);

dxi_R= dl_x(1)*cos(aoa(Nx,end));
dzi_R= -dl_x(1)*sin(aoa(Nx,end))*cos(dih(Nx,end));
vi_R=[dxi_R,dyi,dzi_R]/norm([dxi_R,dyi,dzi_R]);

% for j direction (span wise) left and right are different

dxj_L=dly*sin(Lam(Nx,1))*cos(aoa(Nx,1));

dzj_L=dly*(sin(dih(Nx,1))-sin(aoa(Nx,1))*sin(Lam(Nx,1)));

vj_L=[dxj_L,dyj,dzj_L]/norm([dxj_L,dyj,dzj_L]);

dxj_R= -dly*sin(Lam(Nx,end))*cos(aoa(Nx,end));

dzj_R= -dly*(sin(dih(Nx,end))-sin(aoa(Nx,end))*sin(Lam(Nx,end)));
...
d=-rho*(wake_w_ind+w_ind_d).*dGdx.*repmat(dl_x,Nx,1)*dly + dgdt .* repmat(Sc,Nx,1) .* sin(aoa)  ;
